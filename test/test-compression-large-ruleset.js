...
test("Large Ruleset - Loads successfully", () => {
  ...
  assertTrue(
    largeRuleset.length >= 100,
    `Ruleset should have at least 100 rules (has ${largeRuleset.length})`,
  );
});
...
test("Large Ruleset - Decompression succeeds", async () => {
  ...
  assertTrue(
    ...
    "Decompressed data should have rules property",
  );
  assertTrue(
    Array.isArray(parsed.rules),
    "Decompressed rules should be an array",
  );
});
